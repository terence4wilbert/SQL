...
--Imputed Criteria - Last Years Comparable Period
...
SELECT	lh.Store,DL.Location_No_and_Description,[Labor_Hrs],[Labor_Hrs_LY],[Sales],[Sales_LY],Labor_Exp,Labor_Exp_LY,Total_Donors,Budgets,finalBudget,runningTotal, ULTIPRO_TOTAL
		...
		,case when ((CASE WHEN  finalBudget  IS NOT NULL AND finalBudget <> 0 THEN (Budgets/finalBudget) ELSE 0 END ) * Sales ) <> 0 and finalBudget IS not null then  (((ULTIPRO_TOTAL) - (((CASE WHEN  finalBudget  IS NOT NULL AND finalBudget <> 0 THEN (Budgets/finalBudget) ELSE 0 END ) * Sales ))))  else runningTotal  end [LaborBudgetVarianceCalc]
