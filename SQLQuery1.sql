...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------
SELECT	[Locn] Rev_Locn,Sum(case when [TRXDATE] between @BegDate and @EndDate then [Amount] else 0 end) [accrual]
INTO	#tempRevenueByLocation0101
FROM	[GL00100](NOLOCK) ha /*Header Accounts*/ LEFT JOIN [aztec_GL_Transactions010] gltv(NOLOCK) ON ha.ACTNUMBR_1 = gltv.[AC] JOIN #tempTerritoriesAndStores ts ON ts.Locn_No = gltv.Locn
WHERE	[AC] BETWEEN '698' AND '706' AND Locn = @Locations
		AND ([TRXDATE] BETWEEN  dateadd(YYYY,-1,@BegDate) AND @EndDate) 
GROUP BY [Locn]
ORDER BY [Locn]
...
SELECT	Store,DL.Location_No_and_Description,[Labor_Hrs],[Labor_Hrs_LY] ,[Sales]-- sales amount per store
		...
		,ANNUALS,LATES,BR_LEAVES,accrual
...
FROM	#tempbudget tb,#tempTotalTextileByLocation tk,#tempTotalDonorsByLocation DL  -- Donors by Loc
		...
		left join #tempRevenueByLocation010 ON Store = Rev_Locn left join #tempBudget06 tb6 on tb6.LocnNo = Rev_Locn left join #tempRevenueByLocation0101 tp101 on tp101.Rev_Locn = tb6.BudgetID
...
/*-----------------------------------------[Sales_Per_Labor_Hr]-----------------------------------*/
UNION 
SELECT	Location_No_and_Description,Metric [Description] ,coalesce([Metric_Value],0)[Metric_Value],[Sales_Per_Labor_Hr] [Amt],12 [Seq],'Money' [Value_Format]            ,ISNULL([Sales], 0) AS Sales,ISNULL([Sales_LY], 0) AS Sales_LY,ISNULL(Labor_Exp, 0) AS Labor_Exp,ISNULL(Labor_Exp_LY, 0) AS Labor_Exp_LY,ISNULL([Labor_Hrs], 0) AS Labor_Hrs,ISNULL([Labor_Hrs_LY], 0) AS Labor_Hrs_LY,ISNULL(Total_Donors, 0) AS Total_Donors,Budgets,finalBudget
FROM	#tempLabor_KPIs_010 LEFT JOIN
		#tempMetrics020 ON Metric_AC = '988'
UNION 
SELECT	Location_No_and_Description	,'Labor Hrs LY' [Description]	,0 [Metric_Value],[Labor_Hrs_LY] [Amt],13 [Seq] ,'Decimal' [Value_Format]             ,ISNULL([Sales], 0) AS Sales,ISNULL([Sales_LY], 0) AS Sales_LY,ISNULL(Labor_Exp, 0) AS Labor_Exp,ISNULL(Labor_Exp_LY, 0) AS Labor_Exp_LY,ISNULL([Labor_Hrs], 0) AS Labor_Hrs,ISNULL([Labor_Hrs_LY], 0) AS Labor_Hrs_LY,ISNULL(Total_Donors, 0) AS Total_Donors,Budgets,finalBudget
FROM	#tempLabor_KPIs_010 LEFT JOIN
		#tempMetrics020 ON Metric_AC = '988'		  
UNION 
